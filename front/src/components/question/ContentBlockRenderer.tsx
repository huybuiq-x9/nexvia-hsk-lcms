...
export default function ContentBlockRenderer({ block, className = '' }: Props) {
  ...
  const hasImageMedia = !!block.image_url;
  const hasAudioMedia = !!block.audio_url;
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      {hasImageMedia && block.type === 'text_image_audio' && (
        <img
          src={block.image_url}
          alt={block.image_filename ?? 'image'}
          className="max-w-full max-h-64 rounded-lg border border-slate-200 object-contain"
        />
      )}

      {hasAudioMedia && block.type === 'text_image_audio' && (
        <div className="flex items-center gap-2 bg-slate-50 rounded-lg px-3 py-2 border border-slate-200">
          <Volume2 size={16} className="text-slate-500 shrink-0" />
          <audio controls className="h-8 flex-1 min-w-0" src={block.audio_url}>
            Your browser does not support the audio element.
          </audio>
        </div>
      )}
