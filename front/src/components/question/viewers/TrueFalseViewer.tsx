...
export default function TrueFalseViewer({ question, selected, onChange, readonly, showAnswer }: Props) {
  ...
  // Support both new ('true'/'false') and legacy ('Đúng'/'Sai'/'True') data
  const trueChoice  = question.choices.find(c =>
    c.content.text === 'true' || c.content.text === 'Đúng' || c.content.text === 'True'
  );
  const correctValue = trueChoice?.is_correct === true;
